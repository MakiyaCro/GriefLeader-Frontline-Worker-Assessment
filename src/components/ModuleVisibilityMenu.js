...
const ModuleVisibilityMenu = ({ onToggleModuleVisibility }) => {
  ...
  return (
    <div className={`fixed right-0 top-24 bg-white shadow-lg rounded-l-lg transition-all duration-300 z-50 ${isOpen ? 'w-64' : 'w-10'}`}>
      {/* Toggle Button */}
      <button
        onClick={toggleMenu}
        className="absolute left-0 top-3 w-10 h-10 flex items-center justify-center text-gray-600 hover:text-gray-900 hover:bg-gray-200 rounded-full cursor-pointer z-50"
      >
        {isOpen ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
      </button>

      {/* Menu Content */}
      <div className={`p-4 pl-12 overflow-hidden ${isOpen ? 'opacity-100 block' : 'opacity-0 hidden'}`}>
        <h3 className="text-lg font-semibold mb-4">Module Visibility</h3>
          ...
          {modules.map(module => (
            <li key={module.id} className="flex items-center justify-between">
              ...
              <button
                ...
                className={`p-1 rounded ${moduleStates[module.id] ? 'text-green-600 hover:bg-green-100' : 'text-red-600 hover:bg-red-100'}`}
